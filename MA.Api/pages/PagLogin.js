    ...
    static calls(socket){
        socket.on('LoginIn', async (loginParams) => {
            try {
                ...
                var user = await DUsers.Find(socket.accessDB, 'AND TX_LOGIN = ' + params.addParams(loginParams.login.toUpperCase()), params);
                if (user && user.length > 0){
                    ...
                    // Login no localizado
                    socket.emit('withAccess', false);
                    LogFile.writeLog('PagLogin.LoginIn: Login no localizado: ' + loginParams.login);
                }
            } catch(ex) {
                LogFile.writeLog(`ERROR - PagLogin.LoginIn: ${ex.message}`);
                socket.emit('withAccess', false);
            }
        });
    }
